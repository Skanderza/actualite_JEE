...
//------------------------------------------------------------------------
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        //on filtre le droit d'acces
        //si ok avec user on tag(marque) en session ou cookie l'utilisateur on le fprward
        //si ko redirection
        System.out.println("EntranceServlet::DOGET");
        
        // On FORWARD la requète http(on le pousse plus loin)
        request.getRequestDispatcher("themes?action=0").forward(request, response);
    }
